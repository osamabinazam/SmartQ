...
export default function CompleteQueue() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setCompleteQueue(response.data);
      } catch (error) {
        console.error('Failed to fetch complete queue:', error);
      }
    };
    fetchData();
  }, []);
  ...
  const handleDelete = () => {
    const newData = completeQueue.filter(item => item.queueID !== editedRowData.queueID);
    setCompleteQueue(newData);
